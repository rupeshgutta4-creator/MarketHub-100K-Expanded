'use strict';
// Policy engine for loyaltyAccounts.
const DOMAIN = 'loyaltyAccounts';
const FIELDS = ['id', 'userId', 'points', 'tier', 'lifetimePoints', 'availablePoints', 'createdAt', 'updatedAt'];
...
function requiredRule_2(value) {
  if (!isDefined(value)) return result(false, 'points', 'REQUIRED', 'points'+' is required', value);
  return result(true, 'points', 'OK', 'points'+' is present', value);
}

function trimRule_2(value) {
  if (typeof value !== 'string') return result(true, 'points', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'points', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_2(value) {
  return result(true, 'points', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_2(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'points', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'points', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'points', 'MAX', 'above maximum', n);
  return result(true, 'points', 'OK', 'within range', n);
}

function listRule_2(value) {
  const list = asArray(value);
  return result(true, 'points', 'LIST', 'list normalized', list);
}

function objectRule_2(value) {
  const object = asObject(value);
  return result(true, 'points', 'OBJECT', 'object normalized', object);
}

function requiredRule_3(value) {
  if (!isDefined(value)) return result(false, 'tier', 'REQUIRED', 'tier'+' is required', value);
  return result(true, 'tier', 'OK', 'tier'+' is present', value);
}

function trimRule_3(value) {
  if (typeof value !== 'string') return result(true, 'tier', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'tier', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_3(value) {
  return result(true, 'tier', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_3(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'tier', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'tier', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'tier', 'MAX', 'above maximum', n);
  return result(true, 'tier', 'OK', 'within range', n);
}

function listRule_3(value) {
  const list = asArray(value);
  return result(true, 'tier', 'LIST', 'list normalized', list);
}

function objectRule_3(value) {
  const object = asObject(value);
  return result(true, 'tier', 'OBJECT', 'object normalized', object);
}

function requiredRule_4(value) {
  if (!isDefined(value)) return result(false, 'lifetimePoints', 'REQUIRED', 'lifetimePoints'+' is required', value);
  return result(true, 'lifetimePoints', 'OK', 'lifetimePoints'+' is present', value);
}

function trimRule_4(value) {
  if (typeof value !== 'string') return result(true, 'lifetimePoints', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'lifetimePoints', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_4(value) {
  return result(true, 'lifetimePoints', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_4(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'lifetimePoints', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'lifetimePoints', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'lifetimePoints', 'MAX', 'above maximum', n);
  return result(true, 'lifetimePoints', 'OK', 'within range', n);
}

function listRule_4(value) {
  const list = asArray(value);
  return result(true, 'lifetimePoints', 'LIST', 'list normalized', list);
}

function objectRule_4(value) {
  const object = asObject(value);
  return result(true, 'lifetimePoints', 'OBJECT', 'object normalized', object);
}

function requiredRule_5(value) {
  if (!isDefined(value)) return result(false, 'availablePoints', 'REQUIRED', 'availablePoints'+' is required', value);
  return result(true, 'availablePoints', 'OK', 'availablePoints'+' is present', value);
}

function trimRule_5(value) {
  if (typeof value !== 'string') return result(true, 'availablePoints', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'availablePoints', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_5(value) {
  return result(true, 'availablePoints', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_5(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'availablePoints', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'availablePoints', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'availablePoints', 'MAX', 'above maximum', n);
  return result(true, 'availablePoints', 'OK', 'within range', n);
}

function listRule_5(value) {
  const list = asArray(value);
  return result(true, 'availablePoints', 'LIST', 'list normalized', list);
}

function objectRule_5(value) {
  const object = asObject(value);
  return result(true, 'availablePoints', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'points': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  'tier': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
  'lifetimePoints': Object.freeze([requiredRule_4, trimRule_4, nullRule_4, rangeRule_4, listRule_4, objectRule_4]),
  'availablePoints': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
  'createdAt': Object.freeze([requiredRule_6, trimRule_6, nullRule_6, rangeRule_6, listRule_6, objectRule_6]),
  'updatedAt': Object.freeze([requiredRule_7, trimRule_7, nullRule_7, rangeRule_7, listRule_7, objectRule_7]),
});
