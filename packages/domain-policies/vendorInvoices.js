'use strict';
// Policy engine for vendorInvoices.
const DOMAIN = 'vendorInvoices';
const FIELDS = ['id', 'sellerId', 'periodStart', 'periodEnd', 'subtotal', 'commission', 'tax', 'total', 'status', 'dueAt', 'paidAt', 'createdAt', 'updatedAt'];
...
function requiredRule_5(value) {
  if (!isDefined(value)) return result(false, 'commission', 'REQUIRED', 'commission'+' is required', value);
  return result(true, 'commission', 'OK', 'commission'+' is present', value);
}

function trimRule_5(value) {
  if (typeof value !== 'string') return result(true, 'commission', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'commission', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_5(value) {
  return result(true, 'commission', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_5(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'commission', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'commission', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'commission', 'MAX', 'above maximum', n);
  return result(true, 'commission', 'OK', 'within range', n);
}

function listRule_5(value) {
  const list = asArray(value);
  return result(true, 'commission', 'LIST', 'list normalized', list);
}

function objectRule_5(value) {
  const object = asObject(value);
  return result(true, 'commission', 'OBJECT', 'object normalized', object);
}
...
function requiredRule_9(value) {
  if (!isDefined(value)) return result(false, 'dueAt', 'REQUIRED', 'dueAt'+' is required', value);
  return result(true, 'dueAt', 'OK', 'dueAt'+' is present', value);
}

function trimRule_9(value) {
  if (typeof value !== 'string') return result(true, 'dueAt', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'dueAt', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_9(value) {
  return result(true, 'dueAt', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_9(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'dueAt', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'dueAt', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'dueAt', 'MAX', 'above maximum', n);
  return result(true, 'dueAt', 'OK', 'within range', n);
}

function listRule_9(value) {
  const list = asArray(value);
  return result(true, 'dueAt', 'LIST', 'list normalized', list);
}

function objectRule_9(value) {
  const object = asObject(value);
  return result(true, 'dueAt', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'commission': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
  'tax': Object.freeze([requiredRule_6, trimRule_6, nullRule_6, rangeRule_6, listRule_6, objectRule_6]),
  'total': Object.freeze([requiredRule_7, trimRule_7, nullRule_7, rangeRule_7, listRule_7, objectRule_7]),
  'status': Object.freeze([requiredRule_8, trimRule_8, nullRule_8, rangeRule_8, listRule_8, objectRule_8]),
  'dueAt': Object.freeze([requiredRule_9, trimRule_9, nullRule_9, rangeRule_9, listRule_9, objectRule_9]),
  'paidAt': Object.freeze([requiredRule_10, trimRule_10, nullRule_10, rangeRule_10, listRule_10, objectRule_10]),
