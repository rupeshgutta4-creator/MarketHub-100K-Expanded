'use strict';
// Policy engine for promotions.
const DOMAIN = 'promotions';
const FIELDS = ['id', 'name', 'description', 'type', 'value', 'startsAt', 'endsAt', 'status', 'priority', 'conditions', 'createdAt', 'updatedAt'];
...
function requiredRule_9(value) {
  if (!isDefined(value)) return result(false, 'conditions', 'REQUIRED', 'conditions'+' is required', value);
  return result(true, 'conditions', 'OK', 'conditions'+' is present', value);
}

function trimRule_9(value) {
  if (typeof value !== 'string') return result(true, 'conditions', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'conditions', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_9(value) {
  return result(true, 'conditions', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_9(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'conditions', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'conditions', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'conditions', 'MAX', 'above maximum', n);
  return result(true, 'conditions', 'OK', 'within range', n);
}

function listRule_9(value) {
  const list = asArray(value);
  return result(true, 'conditions', 'LIST', 'list normalized', list);
}

function objectRule_9(value) {
  const object = asObject(value);
  return result(true, 'conditions', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'value': Object.freeze([requiredRule_4, trimRule_4, nullRule_4, rangeRule_4, listRule_4, objectRule_4]),
  'startsAt': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
  'endsAt': Object.freeze([requiredRule_6, trimRule_6, nullRule_6, rangeRule_6, listRule_6, objectRule_6]),
  'status': Object.freeze([requiredRule_7, trimRule_7, nullRule_7, rangeRule_7, listRule_7, objectRule_7]),
  'priority': Object.freeze([requiredRule_8, trimRule_8, nullRule_8, rangeRule_8, listRule_8, objectRule_8]),
  'conditions': Object.freeze([requiredRule_9, trimRule_9, nullRule_9, rangeRule_9, listRule_9, objectRule_9]),
