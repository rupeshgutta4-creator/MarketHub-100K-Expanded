'use strict';
// Policy engine for apiKeys.
const DOMAIN = 'apiKeys';
const FIELDS = ['id', 'userId', 'name', 'keyHash', 'scopes', 'expiresAt', 'lastUsedAt', 'revokedAt', 'createdAt'];
...
function requiredRule_3(value) {
  if (!isDefined(value)) return result(false, 'keyHash', 'REQUIRED', 'keyHash'+' is required', value);
  return result(true, 'keyHash', 'OK', 'keyHash'+' is present', value);
}

function trimRule_3(value) {
  if (typeof value !== 'string') return result(true, 'keyHash', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'keyHash', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_3(value) {
  return result(true, 'keyHash', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_3(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'keyHash', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'keyHash', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'keyHash', 'MAX', 'above maximum', n);
  return result(true, 'keyHash', 'OK', 'within range', n);
}

function listRule_3(value) {
  const list = asArray(value);
  return result(true, 'keyHash', 'LIST', 'list normalized', list);
}

function objectRule_3(value) {
  const object = asObject(value);
  return result(true, 'keyHash', 'OBJECT', 'object normalized', object);
}

function requiredRule_4(value) {
  if (!isDefined(value)) return result(false, 'scopes', 'REQUIRED', 'scopes'+' is required', value);
  return result(true, 'scopes', 'OK', 'scopes'+' is present', value);
}

function trimRule_4(value) {
  if (typeof value !== 'string') return result(true, 'scopes', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'scopes', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_4(value) {
  return result(true, 'scopes', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_4(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'scopes', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'scopes', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'scopes', 'MAX', 'above maximum', n);
  return result(true, 'scopes', 'OK', 'within range', n);
}

function listRule_4(value) {
  const list = asArray(value);
  return result(true, 'scopes', 'LIST', 'list normalized', list);
}

function objectRule_4(value) {
  const object = asObject(value);
  return result(true, 'scopes', 'OBJECT', 'object normalized', object);
}
...
function requiredRule_6(value) {
  if (!isDefined(value)) return result(false, 'lastUsedAt', 'REQUIRED', 'lastUsedAt'+' is required', value);
  return result(true, 'lastUsedAt', 'OK', 'lastUsedAt'+' is present', value);
}

function trimRule_6(value) {
  if (typeof value !== 'string') return result(true, 'lastUsedAt', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'lastUsedAt', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_6(value) {
  return result(true, 'lastUsedAt', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_6(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'lastUsedAt', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'lastUsedAt', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'lastUsedAt', 'MAX', 'above maximum', n);
  return result(true, 'lastUsedAt', 'OK', 'within range', n);
}

function listRule_6(value) {
  const list = asArray(value);
  return result(true, 'lastUsedAt', 'LIST', 'list normalized', list);
}

function objectRule_6(value) {
  const object = asObject(value);
  return result(true, 'lastUsedAt', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'keyHash': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
  'scopes': Object.freeze([requiredRule_4, trimRule_4, nullRule_4, rangeRule_4, listRule_4, objectRule_4]),
  'expiresAt': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
  'lastUsedAt': Object.freeze([requiredRule_6, trimRule_6, nullRule_6, rangeRule_6, listRule_6, objectRule_6]),
  'revokedAt': Object.freeze([requiredRule_7, trimRule_7, nullRule_7, rangeRule_7, listRule_7, objectRule_7]),
