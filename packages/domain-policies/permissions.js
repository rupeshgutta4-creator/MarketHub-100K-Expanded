...
const FIELDS = ['id', 'key', 'resource', 'action', 'description', 'createdAt'];
...
function requiredRule_2(value) {
  if (!isDefined(value)) return result(false, 'resource', 'REQUIRED', 'resource'+' is required', value);
  return result(true, 'resource', 'OK', 'resource'+' is present', value);
}

function trimRule_2(value) {
  if (typeof value !== 'string') return result(true, 'resource', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'resource', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_2(value) {
  return result(true, 'resource', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_2(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'resource', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'resource', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'resource', 'MAX', 'above maximum', n);
  return result(true, 'resource', 'OK', 'within range', n);
}

function listRule_2(value) {
  const list = asArray(value);
  return result(true, 'resource', 'LIST', 'list normalized', list);
}

function objectRule_2(value) {
  const object = asObject(value);
  return result(true, 'resource', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'resource': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  'action': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
