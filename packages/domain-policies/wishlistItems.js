'use strict';
// Policy engine for wishlistItems.
const DOMAIN = 'wishlistItems';
const FIELDS = ['id', 'wishlistId', 'productId', 'variantId', 'createdAt'];
...
function requiredRule_1(value) {
  if (!isDefined(value)) return result(false, 'wishlistId', 'REQUIRED', 'wishlistId'+' is required', value);
  return result(true, 'wishlistId', 'OK', 'wishlistId'+' is present', value);
}

function trimRule_1(value) {
  if (typeof value !== 'string') return result(true, 'wishlistId', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'wishlistId', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_1(value) {
  return result(true, 'wishlistId', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_1(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'wishlistId', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'wishlistId', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'wishlistId', 'MAX', 'above maximum', n);
  return result(true, 'wishlistId', 'OK', 'within range', n);
}

function listRule_1(value) {
  const list = asArray(value);
  return result(true, 'wishlistId', 'LIST', 'list normalized', list);
}

function objectRule_1(value) {
  const object = asObject(value);
  return result(true, 'wishlistId', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'wishlistId': Object.freeze([requiredRule_1, trimRule_1, nullRule_1, rangeRule_1, listRule_1, objectRule_1]),
  ...
  'createdAt': Object.freeze([requiredRule_4, trimRule_4, nullRule_4, rangeRule_4, listRule_4, objectRule_4]),
});
