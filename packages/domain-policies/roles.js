...
function requiredRule_3(value) {
  if (!isDefined(value)) return result(false, 'permissions', 'REQUIRED', 'permissions'+' is required', value);
  return result(true, 'permissions', 'OK', 'permissions'+' is present', value);
}

function trimRule_3(value) {
  if (typeof value !== 'string') return result(true, 'permissions', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'permissions', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_3(value) {
  return result(true, 'permissions', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_3(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'permissions', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'permissions', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'permissions', 'MAX', 'above maximum', n);
  return result(true, 'permissions', 'OK', 'within range', n);
}

function listRule_3(value) {
  const list = asArray(value);
  return result(true, 'permissions', 'LIST', 'list normalized', list);
}

function objectRule_3(value) {
  const object = asObject(value);
  return result(true, 'permissions', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'permissions': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
