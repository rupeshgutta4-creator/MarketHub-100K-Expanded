'use strict';
// Policy engine for productAttributeValues.
const DOMAIN = 'productAttributeValues';
const FIELDS = ['id', 'productId', 'attributeId', 'value', 'createdAt', 'updatedAt'];
...
function requiredRule_2(value) {
  if (!isDefined(value)) return result(false, 'attributeId', 'REQUIRED', 'attributeId'+' is required', value);
  return result(true, 'attributeId', 'OK', 'attributeId'+' is present', value);
}

function trimRule_2(value) {
  if (typeof value !== 'string') return result(true, 'attributeId', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'attributeId', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_2(value) {
  return result(true, 'attributeId', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_2(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'attributeId', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'attributeId', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'attributeId', 'MAX', 'above maximum', n);
  return result(true, 'attributeId', 'OK', 'within range', n);
}

function listRule_2(value) {
  const list = asArray(value);
  return result(true, 'attributeId', 'LIST', 'list normalized', list);
}

function objectRule_2(value) {
  const object = asObject(value);
  return result(true, 'attributeId', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'attributeId': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  ...
  'updatedAt': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
});
