...
function requiredRule_3(value) {
  if (!isDefined(value)) return result(false, 'couponCode', 'REQUIRED', 'couponCode'+' is required', value);
  return result(true, 'couponCode', 'OK', 'couponCode'+' is present', value);
}

function trimRule_3(value) {
  if (typeof value !== 'string') return result(true, 'couponCode', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'couponCode', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_3(value) {
  return result(true, 'couponCode', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_3(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'couponCode', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'couponCode', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'couponCode', 'MAX', 'above maximum', n);
  return result(true, 'couponCode', 'OK', 'within range', n);
}

function listRule_3(value) {
  const list = asArray(value);
  return result(true, 'couponCode', 'LIST', 'list normalized', list);
}

function objectRule_3(value) {
  const object = asObject(value);
  return result(true, 'couponCode', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'buyerId': Object.freeze([requiredRule_1, trimRule_1, nullRule_1, rangeRule_1, listRule_1, objectRule_1]),
  'currency': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  'couponCode': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
