'use strict';
// Policy engine for featureFlags.
const DOMAIN = 'featureFlags';
const FIELDS = ['id', 'key', 'description', 'enabled', 'rules', 'createdAt', 'updatedAt'];
...
function requiredRule_3(value) {
  if (!isDefined(value)) return result(false, 'enabled', 'REQUIRED', 'enabled'+' is required', value);
  return result(true, 'enabled', 'OK', 'enabled'+' is present', value);
}

function trimRule_3(value) {
  if (typeof value !== 'string') return result(true, 'enabled', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'enabled', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_3(value) {
  return result(true, 'enabled', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_3(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'enabled', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'enabled', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'enabled', 'MAX', 'above maximum', n);
  return result(true, 'enabled', 'OK', 'within range', n);
}

function listRule_3(value) {
  const list = asArray(value);
  return result(true, 'enabled', 'LIST', 'list normalized', list);
}

function objectRule_3(value) {
  const object = asObject(value);
  return result(true, 'enabled', 'OBJECT', 'object normalized', object);
}

function requiredRule_4(value) {
  if (!isDefined(value)) return result(false, 'rules', 'REQUIRED', 'rules'+' is required', value);
  return result(true, 'rules', 'OK', 'rules'+' is present', value);
}

function trimRule_4(value) {
  if (typeof value !== 'string') return result(true, 'rules', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'rules', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_4(value) {
  return result(true, 'rules', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_4(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'rules', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'rules', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'rules', 'MAX', 'above maximum', n);
  return result(true, 'rules', 'OK', 'within range', n);
}

function listRule_4(value) {
  const list = asArray(value);
  return result(true, 'rules', 'LIST', 'list normalized', list);
}

function objectRule_4(value) {
  const object = asObject(value);
  return result(true, 'rules', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'description': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  'enabled': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
  'rules': Object.freeze([requiredRule_4, trimRule_4, nullRule_4, rangeRule_4, listRule_4, objectRule_4]),
  'createdAt': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
  'updatedAt': Object.freeze([requiredRule_6, trimRule_6, nullRule_6, rangeRule_6, listRule_6, objectRule_6]),
});
