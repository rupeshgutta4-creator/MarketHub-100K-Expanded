...
function requiredRule_2(value) {
  if (!isDefined(value)) return result(false, 'tokenHash', 'REQUIRED', 'tokenHash'+' is required', value);
  return result(true, 'tokenHash', 'OK', 'tokenHash'+' is present', value);
}

function trimRule_2(value) {
  if (typeof value !== 'string') return result(true, 'tokenHash', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'tokenHash', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_2(value) {
  return result(true, 'tokenHash', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_2(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'tokenHash', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'tokenHash', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'tokenHash', 'MAX', 'above maximum', n);
  return result(true, 'tokenHash', 'OK', 'within range', n);
}

function listRule_2(value) {
  const list = asArray(value);
  return result(true, 'tokenHash', 'LIST', 'list normalized', list);
}

function objectRule_2(value) {
  const object = asObject(value);
  return result(true, 'tokenHash', 'OBJECT', 'object normalized', object);
}
...
function requiredRule_6(value) {
  if (!isDefined(value)) return result(false, 'revokedAt', 'REQUIRED', 'revokedAt'+' is required', value);
  return result(true, 'revokedAt', 'OK', 'revokedAt'+' is present', value);
}

function trimRule_6(value) {
  if (typeof value !== 'string') return result(true, 'revokedAt', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'revokedAt', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_6(value) {
  return result(true, 'revokedAt', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_6(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'revokedAt', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'revokedAt', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'revokedAt', 'MAX', 'above maximum', n);
  return result(true, 'revokedAt', 'OK', 'within range', n);
}

function listRule_6(value) {
  const list = asArray(value);
  return result(true, 'revokedAt', 'LIST', 'list normalized', list);
}

function objectRule_6(value) {
  const object = asObject(value);
  return result(true, 'revokedAt', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'tokenHash': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  'ipAddress': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
  'userAgent': Object.freeze([requiredRule_4, trimRule_4, nullRule_4, rangeRule_4, listRule_4, objectRule_4]),
  'expiresAt': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
  'revokedAt': Object.freeze([requiredRule_6, trimRule_6, nullRule_6, rangeRule_6, listRule_6, objectRule_6]),
