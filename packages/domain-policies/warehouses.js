'use strict';
// Policy engine for warehouses.
const DOMAIN = 'warehouses';
const FIELDS = ['id', 'name', 'code', 'address', 'city', 'state', 'country', 'postalCode', 'phone', 'managerId', 'status', 'createdAt', 'updatedAt'];
...
function requiredRule_3(value) {
  if (!isDefined(value)) return result(false, 'address', 'REQUIRED', 'address'+' is required', value);
  return result(true, 'address', 'OK', 'address'+' is present', value);
}

function trimRule_3(value) {
  if (typeof value !== 'string') return result(true, 'address', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'address', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_3(value) {
  return result(true, 'address', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_3(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'address', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'address', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'address', 'MAX', 'above maximum', n);
  return result(true, 'address', 'OK', 'within range', n);
}

function listRule_3(value) {
  const list = asArray(value);
  return result(true, 'address', 'LIST', 'list normalized', list);
}

function objectRule_3(value) {
  const object = asObject(value);
  return result(true, 'address', 'OBJECT', 'object normalized', object);
}
...
function requiredRule_9(value) {
  if (!isDefined(value)) return result(false, 'managerId', 'REQUIRED', 'managerId'+' is required', value);
  return result(true, 'managerId', 'OK', 'managerId'+' is present', value);
}

function trimRule_9(value) {
  if (typeof value !== 'string') return result(true, 'managerId', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'managerId', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_9(value) {
  return result(true, 'managerId', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_9(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'managerId', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'managerId', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'managerId', 'MAX', 'above maximum', n);
  return result(true, 'managerId', 'OK', 'within range', n);
}

function listRule_9(value) {
  const list = asArray(value);
  return result(true, 'managerId', 'LIST', 'list normalized', list);
}

function objectRule_9(value) {
  const object = asObject(value);
  return result(true, 'managerId', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'code': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  'address': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
  'city': Object.freeze([requiredRule_4, trimRule_4, nullRule_4, rangeRule_4, listRule_4, objectRule_4]),
  'state': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
  'country': Object.freeze([requiredRule_6, trimRule_6, nullRule_6, rangeRule_6, listRule_6, objectRule_6]),
  'postalCode': Object.freeze([requiredRule_7, trimRule_7, nullRule_7, rangeRule_7, listRule_7, objectRule_7]),
  'phone': Object.freeze([requiredRule_8, trimRule_8, nullRule_8, rangeRule_8, listRule_8, objectRule_8]),
  'managerId': Object.freeze([requiredRule_9, trimRule_9, nullRule_9, rangeRule_9, listRule_9, objectRule_9]),
