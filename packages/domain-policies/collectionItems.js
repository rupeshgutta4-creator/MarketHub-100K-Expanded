'use strict';
// Policy engine for collectionItems.
const DOMAIN = 'collectionItems';
const FIELDS = ['id', 'collectionId', 'productId', 'position', 'createdAt'];
...
function requiredRule_1(value) {
  if (!isDefined(value)) return result(false, 'collectionId', 'REQUIRED', 'collectionId'+' is required', value);
  return result(true, 'collectionId', 'OK', 'collectionId'+' is present', value);
}

function trimRule_1(value) {
  if (typeof value !== 'string') return result(true, 'collectionId', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'collectionId', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_1(value) {
  return result(true, 'collectionId', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_1(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'collectionId', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'collectionId', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'collectionId', 'MAX', 'above maximum', n);
  return result(true, 'collectionId', 'OK', 'within range', n);
}

function listRule_1(value) {
  const list = asArray(value);
  return result(true, 'collectionId', 'LIST', 'list normalized', list);
}

function objectRule_1(value) {
  const object = asObject(value);
  return result(true, 'collectionId', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'collectionId': Object.freeze([requiredRule_1, trimRule_1, nullRule_1, rangeRule_1, listRule_1, objectRule_1]),
  'productId': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  'position': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
