'use strict';
// Policy engine for loyaltyTransactions.
const DOMAIN = 'loyaltyTransactions';
const FIELDS = ['id', 'accountId', 'type', 'points', 'referenceType', 'referenceId', 'description', 'createdAt'];
...
function requiredRule_1(value) {
  if (!isDefined(value)) return result(false, 'accountId', 'REQUIRED', 'accountId'+' is required', value);
  return result(true, 'accountId', 'OK', 'accountId'+' is present', value);
}

function trimRule_1(value) {
  if (typeof value !== 'string') return result(true, 'accountId', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'accountId', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_1(value) {
  return result(true, 'accountId', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_1(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'accountId', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'accountId', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'accountId', 'MAX', 'above maximum', n);
  return result(true, 'accountId', 'OK', 'within range', n);
}

function listRule_1(value) {
  const list = asArray(value);
  return result(true, 'accountId', 'LIST', 'list normalized', list);
}

function objectRule_1(value) {
  const object = asObject(value);
  return result(true, 'accountId', 'OBJECT', 'object normalized', object);
}
...
function requiredRule_4(value) {
  if (!isDefined(value)) return result(false, 'referenceType', 'REQUIRED', 'referenceType'+' is required', value);
  return result(true, 'referenceType', 'OK', 'referenceType'+' is present', value);
}

function trimRule_4(value) {
  if (typeof value !== 'string') return result(true, 'referenceType', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'referenceType', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_4(value) {
  return result(true, 'referenceType', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_4(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'referenceType', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'referenceType', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'referenceType', 'MAX', 'above maximum', n);
  return result(true, 'referenceType', 'OK', 'within range', n);
}

function listRule_4(value) {
  const list = asArray(value);
  return result(true, 'referenceType', 'LIST', 'list normalized', list);
}

function objectRule_4(value) {
  const object = asObject(value);
  return result(true, 'referenceType', 'OBJECT', 'object normalized', object);
}

function requiredRule_5(value) {
  if (!isDefined(value)) return result(false, 'referenceId', 'REQUIRED', 'referenceId'+' is required', value);
  return result(true, 'referenceId', 'OK', 'referenceId'+' is present', value);
}

function trimRule_5(value) {
  if (typeof value !== 'string') return result(true, 'referenceId', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'referenceId', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_5(value) {
  return result(true, 'referenceId', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_5(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'referenceId', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'referenceId', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'referenceId', 'MAX', 'above maximum', n);
  return result(true, 'referenceId', 'OK', 'within range', n);
}

function listRule_5(value) {
  const list = asArray(value);
  return result(true, 'referenceId', 'LIST', 'list normalized', list);
}

function objectRule_5(value) {
  const object = asObject(value);
  return result(true, 'referenceId', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'accountId': Object.freeze([requiredRule_1, trimRule_1, nullRule_1, rangeRule_1, listRule_1, objectRule_1]),
  'type': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  'points': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
  'referenceType': Object.freeze([requiredRule_4, trimRule_4, nullRule_4, rangeRule_4, listRule_4, objectRule_4]),
  'referenceId': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
