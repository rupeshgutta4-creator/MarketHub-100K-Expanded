'use strict';
// Policy engine for shippingZones.
const DOMAIN = 'shippingZones';
const FIELDS = ['id', 'name', 'countries', 'states', 'postalCodes', 'active', 'createdAt', 'updatedAt'];
...
function requiredRule_2(value) {
  if (!isDefined(value)) return result(false, 'countries', 'REQUIRED', 'countries'+' is required', value);
  return result(true, 'countries', 'OK', 'countries'+' is present', value);
}

function trimRule_2(value) {
  if (typeof value !== 'string') return result(true, 'countries', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'countries', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_2(value) {
  return result(true, 'countries', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_2(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'countries', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'countries', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'countries', 'MAX', 'above maximum', n);
  return result(true, 'countries', 'OK', 'within range', n);
}

function listRule_2(value) {
  const list = asArray(value);
  return result(true, 'countries', 'LIST', 'list normalized', list);
}

function objectRule_2(value) {
  const object = asObject(value);
  return result(true, 'countries', 'OBJECT', 'object normalized', object);
}

function requiredRule_3(value) {
  if (!isDefined(value)) return result(false, 'states', 'REQUIRED', 'states'+' is required', value);
  return result(true, 'states', 'OK', 'states'+' is present', value);
}

function trimRule_3(value) {
  if (typeof value !== 'string') return result(true, 'states', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'states', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_3(value) {
  return result(true, 'states', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_3(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'states', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'states', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'states', 'MAX', 'above maximum', n);
  return result(true, 'states', 'OK', 'within range', n);
}

function listRule_3(value) {
  const list = asArray(value);
  return result(true, 'states', 'LIST', 'list normalized', list);
}

function objectRule_3(value) {
  const object = asObject(value);
  return result(true, 'states', 'OBJECT', 'object normalized', object);
}

function requiredRule_4(value) {
  if (!isDefined(value)) return result(false, 'postalCodes', 'REQUIRED', 'postalCodes'+' is required', value);
  return result(true, 'postalCodes', 'OK', 'postalCodes'+' is present', value);
}

function trimRule_4(value) {
  if (typeof value !== 'string') return result(true, 'postalCodes', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'postalCodes', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_4(value) {
  return result(true, 'postalCodes', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_4(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'postalCodes', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'postalCodes', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'postalCodes', 'MAX', 'above maximum', n);
  return result(true, 'postalCodes', 'OK', 'within range', n);
}

function listRule_4(value) {
  const list = asArray(value);
  return result(true, 'postalCodes', 'LIST', 'list normalized', list);
}

function objectRule_4(value) {
  const object = asObject(value);
  return result(true, 'postalCodes', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'countries': Object.freeze([requiredRule_2, trimRule_2, nullRule_2, rangeRule_2, listRule_2, objectRule_2]),
  'states': Object.freeze([requiredRule_3, trimRule_3, nullRule_3, rangeRule_3, listRule_3, objectRule_3]),
  'postalCodes': Object.freeze([requiredRule_4, trimRule_4, nullRule_4, rangeRule_4, listRule_4, objectRule_4]),
  'active': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
