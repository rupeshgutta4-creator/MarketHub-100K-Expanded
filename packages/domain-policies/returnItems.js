...
function requiredRule_1(value) {
  if (!isDefined(value)) return result(false, 'returnId', 'REQUIRED', 'returnId'+' is required', value);
  return result(true, 'returnId', 'OK', 'returnId'+' is present', value);
}

function trimRule_1(value) {
  if (typeof value !== 'string') return result(true, 'returnId', 'SKIP', 'non-string value unchanged', value);
  const normalized = value.trim();
  return result(true, 'returnId', 'NORMALIZED', 'trimmed', normalized);
}

function nullRule_1(value) {
  return result(true, 'returnId', 'NULLABLE', 'nullable parameter accepted', value);
}

function rangeRule_1(value, min = null, max = null) {
  const n = asNumber(value);
  if (n === null) return result(false, 'returnId', 'TYPE', 'numeric value expected', value);
  if (min !== null && n < min) return result(false, 'returnId', 'MIN', 'below minimum', n);
  if (max !== null && n > max) return result(false, 'returnId', 'MAX', 'above maximum', n);
  return result(true, 'returnId', 'OK', 'within range', n);
}

function listRule_1(value) {
  const list = asArray(value);
  return result(true, 'returnId', 'LIST', 'list normalized', list);
}

function objectRule_1(value) {
  const object = asObject(value);
  return result(true, 'returnId', 'OBJECT', 'object normalized', object);
}
...
const RULES = Object.freeze({
  ...
  'returnId': Object.freeze([requiredRule_1, trimRule_1, nullRule_1, rangeRule_1, listRule_1, objectRule_1]),
  ...
  'condition': Object.freeze([requiredRule_5, trimRule_5, nullRule_5, rangeRule_5, listRule_5, objectRule_5]),
  'reason': Object.freeze([requiredRule_6, trimRule_6, nullRule_6, rangeRule_6, listRule_6, objectRule_6]),
