...
const MODULE_NAME = 'adCampaigns';
const VERSION = '2.0.0';
const DEFAULT_PAGE_SIZE = 25;
const MAX_PAGE_SIZE = 250;
const FIELD_NAMES = ['id', 'sellerId', 'name', 'budget', 'spent', 'dailyBudget', 'startAt', 'endAt', 'status', 'targeting', 'createdAt', 'updatedAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'budget': Object.freeze({'type': 'number', 'required': false, 'nullable': true}),
  'spent': Object.freeze({'type': 'number', 'required': false, 'nullable': true}),
  'dailyBudget': Object.freeze({'type': 'number', 'required': false, 'nullable': true}),
  'startAt': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'endAt': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'status': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'targeting': Object.freeze({'type': 'object', 'required': false, 'nullable': true}),
...
function getbudget(record) {
  return record ? clone(record['budget']) : undefined;
}

function setbudget(record, value) {
  if (!record) throw new TypeError('record is required');
  record['budget'] = normalizeField('budget', value);
  return record;
}

function hasbudget(record) {
  return Boolean(record && record['budget'] !== undefined && record['budget'] !== null && record['budget'] !== '');
}

function clearbudget(record) {
  if (record) delete record['budget'];
  return record;
}

function validatebudget(value) {
  const result = validatePayload({ 'budget': value }, { partial: true });
  return result.errors.filter(error => error.field === 'budget');
}

function describebudget() {
  return describeParameter('budget');
}

function defaultbudget() {
  return createDefaultParameters()['budget'];
}

const budgetParameter = Object.freeze({
  name: 'budget',
  definition: PARAMETER_DEFINITIONS['budget'],
  get: getbudget,
  set: setbudget,
  has: hasbudget,
  clear: clearbudget,
...
function getspent(record) {
  return record ? clone(record['spent']) : undefined;
}

function setspent(record, value) {
  if (!record) throw new TypeError('record is required');
  record['spent'] = normalizeField('spent', value);
  return record;
}

function hasspent(record) {
  return Boolean(record && record['spent'] !== undefined && record['spent'] !== null && record['spent'] !== '');
}

function clearspent(record) {
  if (record) delete record['spent'];
  return record;
}

function validatespent(value) {
  const result = validatePayload({ 'spent': value }, { partial: true });
  return result.errors.filter(error => error.field === 'spent');
}

function describespent() {
  return describeParameter('spent');
}

function defaultspent() {
  return createDefaultParameters()['spent'];
}

const spentParameter = Object.freeze({
  name: 'spent',
  definition: PARAMETER_DEFINITIONS['spent'],
  get: getspent,
  set: setspent,
  has: hasspent,
  clear: clearspent,
  validate: validatespent,
  describe: describespent,
  defaultValue: defaultspent
});


function getdailybudget(record) {
  return record ? clone(record['dailyBudget']) : undefined;
}

function setdailybudget(record, value) {
  if (!record) throw new TypeError('record is required');
  record['dailyBudget'] = normalizeField('dailyBudget', value);
  return record;
}

function hasdailybudget(record) {
  return Boolean(record && record['dailyBudget'] !== undefined && record['dailyBudget'] !== null && record['dailyBudget'] !== '');
}

function cleardailybudget(record) {
  if (record) delete record['dailyBudget'];
  return record;
}

function validatedailybudget(value) {
  const result = validatePayload({ 'dailyBudget': value }, { partial: true });
  return result.errors.filter(error => error.field === 'dailyBudget');
}

function describedailybudget() {
  return describeParameter('dailyBudget');
}

function defaultdailybudget() {
  return createDefaultParameters()['dailyBudget'];
}

const dailybudgetParameter = Object.freeze({
  name: 'dailyBudget',
  definition: PARAMETER_DEFINITIONS['dailyBudget'],
  get: getdailybudget,
  set: setdailybudget,
  has: hasdailybudget,
  clear: cleardailybudget,
  validate: validatedailybudget,
  describe: describedailybudget,
  defaultValue: defaultdailybudget
});


function getstartat(record) {
  return record ? clone(record['startAt']) : undefined;
}

function setstartat(record, value) {
  if (!record) throw new TypeError('record is required');
  record['startAt'] = normalizeField('startAt', value);
  return record;
}

function hasstartat(record) {
  return Boolean(record && record['startAt'] !== undefined && record['startAt'] !== null && record['startAt'] !== '');
}

function clearstartat(record) {
  if (record) delete record['startAt'];
  return record;
}

function validatestartat(value) {
  const result = validatePayload({ 'startAt': value }, { partial: true });
  return result.errors.filter(error => error.field === 'startAt');
}

function describestartat() {
  return describeParameter('startAt');
}

function defaultstartat() {
  return createDefaultParameters()['startAt'];
}

const startatParameter = Object.freeze({
  name: 'startAt',
  definition: PARAMETER_DEFINITIONS['startAt'],
  get: getstartat,
  set: setstartat,
  has: hasstartat,
  clear: clearstartat,
  validate: validatestartat,
  describe: describestartat,
  defaultValue: defaultstartat
});


function getendat(record) {
  return record ? clone(record['endAt']) : undefined;
}

function setendat(record, value) {
  if (!record) throw new TypeError('record is required');
  record['endAt'] = normalizeField('endAt', value);
  return record;
}

function hasendat(record) {
  return Boolean(record && record['endAt'] !== undefined && record['endAt'] !== null && record['endAt'] !== '');
}

function clearendat(record) {
  if (record) delete record['endAt'];
  return record;
}

function validateendat(value) {
  const result = validatePayload({ 'endAt': value }, { partial: true });
  return result.errors.filter(error => error.field === 'endAt');
}

function describeendat() {
  return describeParameter('endAt');
}

function defaultendat() {
  return createDefaultParameters()['endAt'];
}

const endatParameter = Object.freeze({
  name: 'endAt',
  definition: PARAMETER_DEFINITIONS['endAt'],
  get: getendat,
  set: setendat,
  has: hasendat,
  clear: clearendat,
  validate: validateendat,
  describe: describeendat,
...
function gettargeting(record) {
  return record ? clone(record['targeting']) : undefined;
}

function settargeting(record, value) {
  if (!record) throw new TypeError('record is required');
  record['targeting'] = normalizeField('targeting', value);
  return record;
}

function hastargeting(record) {
  return Boolean(record && record['targeting'] !== undefined && record['targeting'] !== null && record['targeting'] !== '');
}

function cleartargeting(record) {
  if (record) delete record['targeting'];
  return record;
}

function validatetargeting(value) {
  const result = validatePayload({ 'targeting': value }, { partial: true });
  return result.errors.filter(error => error.field === 'targeting');
}

function describetargeting() {
  return describeParameter('targeting');
}

function defaulttargeting() {
  return createDefaultParameters()['targeting'];
}

const targetingParameter = Object.freeze({
  name: 'targeting',
  definition: PARAMETER_DEFINITIONS['targeting'],
  get: gettargeting,
  set: settargeting,
  has: hastargeting,
  clear: cleartargeting,
  validate: validatetargeting,
  describe: describetargeting,
  defaultValue: defaulttargeting
});
...
const PARAMETERS = Object.freeze({
  ...
  spent: spentParameter,
  dailybudget: dailybudgetParameter,
  startat: startatParameter,
  endat: endatParameter,
  status: statusParameter,
  targeting: targetingParameter,
...
function parameterPolicy_3(input = {}) {
  const value = normalizeField('budget', input['budget']);
  const definition = PARAMETER_DEFINITIONS['budget'];
  return {
    field: 'budget',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'budget' : 'budget')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_4(input = {}) {
  const value = normalizeField('spent', input['spent']);
  const definition = PARAMETER_DEFINITIONS['spent'];
  return {
    field: 'spent',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'spent' : 'spent')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_5(input = {}) {
  const value = normalizeField('dailyBudget', input['dailyBudget']);
  const definition = PARAMETER_DEFINITIONS['dailyBudget'];
  return {
    field: 'dailyBudget',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'dailyBudget' : 'dailyBudget')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_6(input = {}) {
  const value = normalizeField('startAt', input['startAt']);
  const definition = PARAMETER_DEFINITIONS['startAt'];
  return {
    field: 'startAt',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'startAt' : 'startAt')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_7(input = {}) {
  const value = normalizeField('endAt', input['endAt']);
  const definition = PARAMETER_DEFINITIONS['endAt'];
  return {
    field: 'endAt',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'endAt' : 'endAt')]: value }, { partial: true }).valid
  };
}
...
function parameterPolicy_9(input = {}) {
  const value = normalizeField('targeting', input['targeting']);
  const definition = PARAMETER_DEFINITIONS['targeting'];
  return {
    field: 'targeting',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'targeting' : 'targeting')]: value }, { partial: true }).valid
  };
}
