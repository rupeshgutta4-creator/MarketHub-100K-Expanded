...
const FIELD_NAMES = ['id', 'actorId', 'action', 'entity', 'entityId', 'metadata', 'ipAddress', 'userAgent', 'createdAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'actorId': Object.freeze({'type': 'string', 'required': true, 'nullable': false}),
  'action': Object.freeze({'type': 'string', 'required': true, 'nullable': false}),
  'entity': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
...
function getactorid(record) {
  return record ? clone(record['actorId']) : undefined;
}

function setactorid(record, value) {
  if (!record) throw new TypeError('record is required');
  record['actorId'] = normalizeField('actorId', value);
  return record;
}

function hasactorid(record) {
  return Boolean(record && record['actorId'] !== undefined && record['actorId'] !== null && record['actorId'] !== '');
}

function clearactorid(record) {
  if (record) delete record['actorId'];
  return record;
}

function validateactorid(value) {
  const result = validatePayload({ 'actorId': value }, { partial: true });
  return result.errors.filter(error => error.field === 'actorId');
}

function describeactorid() {
  return describeParameter('actorId');
}

function defaultactorid() {
  return createDefaultParameters()['actorId'];
}

const actoridParameter = Object.freeze({
  name: 'actorId',
  definition: PARAMETER_DEFINITIONS['actorId'],
  get: getactorid,
  set: setactorid,
  has: hasactorid,
  clear: clearactorid,
  validate: validateactorid,
  describe: describeactorid,
...
function getentity(record) {
  return record ? clone(record['entity']) : undefined;
}

function setentity(record, value) {
  if (!record) throw new TypeError('record is required');
  record['entity'] = normalizeField('entity', value);
  return record;
}

function hasentity(record) {
  return Boolean(record && record['entity'] !== undefined && record['entity'] !== null && record['entity'] !== '');
}

function clearentity(record) {
  if (record) delete record['entity'];
  return record;
}

function validateentity(value) {
  const result = validatePayload({ 'entity': value }, { partial: true });
  return result.errors.filter(error => error.field === 'entity');
}

function describeentity() {
  return describeParameter('entity');
}

function defaultentity() {
  return createDefaultParameters()['entity'];
}

const entityParameter = Object.freeze({
  name: 'entity',
  definition: PARAMETER_DEFINITIONS['entity'],
  get: getentity,
  set: setentity,
  has: hasentity,
  clear: clearentity,
...
function parameterPolicy_1(input = {}) {
  const value = normalizeField('actorId', input['actorId']);
  const definition = PARAMETER_DEFINITIONS['actorId'];
  return {
    field: 'actorId',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'actorId' : 'actorId')]: value }, { partial: true }).valid
  };
}
...
function parameterPolicy_3(input = {}) {
  const value = normalizeField('entity', input['entity']);
  const definition = PARAMETER_DEFINITIONS['entity'];
  return {
    field: 'entity',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'entity' : 'entity')]: value }, { partial: true }).valid
  };
}
