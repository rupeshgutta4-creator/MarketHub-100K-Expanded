...
const FIELD_NAMES = ['id', 'key', 'value', 'type', 'scope', 'description', 'isSecret', 'createdAt', 'updatedAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'value': Object.freeze({'type': 'number', 'required': true, 'nullable': false}),
  'type': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'scope': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'description': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'isSecret': Object.freeze({'type': 'boolean', 'required': false, 'nullable': true}),
...
function getscope(record) {
  return record ? clone(record['scope']) : undefined;
}

function setscope(record, value) {
  if (!record) throw new TypeError('record is required');
  record['scope'] = normalizeField('scope', value);
  return record;
}

function hasscope(record) {
  return Boolean(record && record['scope'] !== undefined && record['scope'] !== null && record['scope'] !== '');
}

function clearscope(record) {
  if (record) delete record['scope'];
  return record;
}

function validatescope(value) {
  const result = validatePayload({ 'scope': value }, { partial: true });
  return result.errors.filter(error => error.field === 'scope');
}

function describescope() {
  return describeParameter('scope');
}

function defaultscope() {
  return createDefaultParameters()['scope'];
}

const scopeParameter = Object.freeze({
  name: 'scope',
  definition: PARAMETER_DEFINITIONS['scope'],
  get: getscope,
  set: setscope,
  has: hasscope,
  clear: clearscope,
...
function getissecret(record) {
  return record ? clone(record['isSecret']) : undefined;
}

function setissecret(record, value) {
  if (!record) throw new TypeError('record is required');
  record['isSecret'] = normalizeField('isSecret', value);
  return record;
}

function hasissecret(record) {
  return Boolean(record && record['isSecret'] !== undefined && record['isSecret'] !== null && record['isSecret'] !== '');
}

function clearissecret(record) {
  if (record) delete record['isSecret'];
  return record;
}

function validateissecret(value) {
  const result = validatePayload({ 'isSecret': value }, { partial: true });
  return result.errors.filter(error => error.field === 'isSecret');
}

function describeissecret() {
  return describeParameter('isSecret');
}

function defaultissecret() {
  return createDefaultParameters()['isSecret'];
}

const issecretParameter = Object.freeze({
  name: 'isSecret',
  definition: PARAMETER_DEFINITIONS['isSecret'],
  get: getissecret,
  set: setissecret,
  has: hasissecret,
  clear: clearissecret,
  validate: validateissecret,
  describe: describeissecret,
  defaultValue: defaultissecret
});
...
function parameterPolicy_4(input = {}) {
  const value = normalizeField('scope', input['scope']);
  const definition = PARAMETER_DEFINITIONS['scope'];
  return {
    field: 'scope',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'scope' : 'scope')]: value }, { partial: true }).valid
  };
}
...
function parameterPolicy_6(input = {}) {
  const value = normalizeField('isSecret', input['isSecret']);
  const definition = PARAMETER_DEFINITIONS['isSecret'];
  return {
    field: 'isSecret',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'isSecret' : 'isSecret')]: value }, { partial: true }).valid
  };
}
