...
const MODULE_NAME = 'wishlistItems';
const VERSION = '2.0.0';
const DEFAULT_PAGE_SIZE = 25;
const MAX_PAGE_SIZE = 250;
const FIELD_NAMES = ['id', 'wishlistId', 'productId', 'variantId', 'createdAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'wishlistId': Object.freeze({'type': 'string', 'required': true, 'nullable': false}),
...
function getwishlistid(record) {
  return record ? clone(record['wishlistId']) : undefined;
}

function setwishlistid(record, value) {
  if (!record) throw new TypeError('record is required');
  record['wishlistId'] = normalizeField('wishlistId', value);
  return record;
}

function haswishlistid(record) {
  return Boolean(record && record['wishlistId'] !== undefined && record['wishlistId'] !== null && record['wishlistId'] !== '');
}

function clearwishlistid(record) {
  if (record) delete record['wishlistId'];
  return record;
}

function validatewishlistid(value) {
  const result = validatePayload({ 'wishlistId': value }, { partial: true });
  return result.errors.filter(error => error.field === 'wishlistId');
}

function describewishlistid() {
  return describeParameter('wishlistId');
}

function defaultwishlistid() {
  return createDefaultParameters()['wishlistId'];
}

const wishlistidParameter = Object.freeze({
  name: 'wishlistId',
  definition: PARAMETER_DEFINITIONS['wishlistId'],
  get: getwishlistid,
  set: setwishlistid,
  has: haswishlistid,
  clear: clearwishlistid,
  validate: validatewishlistid,
  describe: describewishlistid,
  defaultValue: defaultwishlistid
});
...
const PARAMETERS = Object.freeze({
  id: idParameter,
  wishlistid: wishlistidParameter,
...
function parameterPolicy_1(input = {}) {
  const value = normalizeField('wishlistId', input['wishlistId']);
  const definition = PARAMETER_DEFINITIONS['wishlistId'];
  return {
    field: 'wishlistId',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'wishlistId' : 'wishlistId')]: value }, { partial: true }).valid
  };
}
