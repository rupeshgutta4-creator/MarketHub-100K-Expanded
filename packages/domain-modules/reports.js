...
const FIELD_NAMES = ['id', 'name', 'type', 'parameters', 'format', 'status', 'fileUrl', 'requestedBy', 'generatedAt', 'createdAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'parameters': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'format': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'status': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'fileUrl': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'requestedBy': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'generatedAt': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
...
function getparameters(record) {
  return record ? clone(record['parameters']) : undefined;
}

function setparameters(record, value) {
  if (!record) throw new TypeError('record is required');
  record['parameters'] = normalizeField('parameters', value);
  return record;
}

function hasparameters(record) {
  return Boolean(record && record['parameters'] !== undefined && record['parameters'] !== null && record['parameters'] !== '');
}

function clearparameters(record) {
  if (record) delete record['parameters'];
  return record;
}

function validateparameters(value) {
  const result = validatePayload({ 'parameters': value }, { partial: true });
  return result.errors.filter(error => error.field === 'parameters');
}

function describeparameters() {
  return describeParameter('parameters');
}

function defaultparameters() {
  return createDefaultParameters()['parameters'];
}

const parametersParameter = Object.freeze({
  name: 'parameters',
...
function getformat(record) {
  return record ? clone(record['format']) : undefined;
}

function setformat(record, value) {
  if (!record) throw new TypeError('record is required');
  record['format'] = normalizeField('format', value);
  return record;
}

function hasformat(record) {
  return Boolean(record && record['format'] !== undefined && record['format'] !== null && record['format'] !== '');
}

function clearformat(record) {
  if (record) delete record['format'];
  return record;
}

function validateformat(value) {
  const result = validatePayload({ 'format': value }, { partial: true });
  return result.errors.filter(error => error.field === 'format');
}

function describeformat() {
  return describeParameter('format');
}

function defaultformat() {
  return createDefaultParameters()['format'];
}

const formatParameter = Object.freeze({
  name: 'format',
  definition: PARAMETER_DEFINITIONS['format'],
  get: getformat,
  set: setformat,
  has: hasformat,
  clear: clearformat,
...
function getfileurl(record) {
  return record ? clone(record['fileUrl']) : undefined;
}

function setfileurl(record, value) {
  if (!record) throw new TypeError('record is required');
  record['fileUrl'] = normalizeField('fileUrl', value);
  return record;
}

function hasfileurl(record) {
  return Boolean(record && record['fileUrl'] !== undefined && record['fileUrl'] !== null && record['fileUrl'] !== '');
}

function clearfileurl(record) {
  if (record) delete record['fileUrl'];
  return record;
}

function validatefileurl(value) {
  const result = validatePayload({ 'fileUrl': value }, { partial: true });
  return result.errors.filter(error => error.field === 'fileUrl');
}

function describefileurl() {
  return describeParameter('fileUrl');
}

function defaultfileurl() {
  return createDefaultParameters()['fileUrl'];
}

const fileurlParameter = Object.freeze({
  name: 'fileUrl',
  definition: PARAMETER_DEFINITIONS['fileUrl'],
  get: getfileurl,
  set: setfileurl,
  has: hasfileurl,
  clear: clearfileurl,
  validate: validatefileurl,
  describe: describefileurl,
  defaultValue: defaultfileurl
});


function getrequestedby(record) {
  return record ? clone(record['requestedBy']) : undefined;
}

function setrequestedby(record, value) {
  if (!record) throw new TypeError('record is required');
  record['requestedBy'] = normalizeField('requestedBy', value);
  return record;
}

function hasrequestedby(record) {
  return Boolean(record && record['requestedBy'] !== undefined && record['requestedBy'] !== null && record['requestedBy'] !== '');
}

function clearrequestedby(record) {
  if (record) delete record['requestedBy'];
  return record;
}

function validaterequestedby(value) {
  const result = validatePayload({ 'requestedBy': value }, { partial: true });
  return result.errors.filter(error => error.field === 'requestedBy');
}

function describerequestedby() {
  return describeParameter('requestedBy');
}

function defaultrequestedby() {
  return createDefaultParameters()['requestedBy'];
}

const requestedbyParameter = Object.freeze({
  name: 'requestedBy',
  definition: PARAMETER_DEFINITIONS['requestedBy'],
  get: getrequestedby,
  set: setrequestedby,
  has: hasrequestedby,
  clear: clearrequestedby,
  validate: validaterequestedby,
  describe: describerequestedby,
  defaultValue: defaultrequestedby
});


function getgeneratedat(record) {
  return record ? clone(record['generatedAt']) : undefined;
}

function setgeneratedat(record, value) {
  if (!record) throw new TypeError('record is required');
  record['generatedAt'] = normalizeField('generatedAt', value);
  return record;
}

function hasgeneratedat(record) {
  return Boolean(record && record['generatedAt'] !== undefined && record['generatedAt'] !== null && record['generatedAt'] !== '');
}

function cleargeneratedat(record) {
  if (record) delete record['generatedAt'];
  return record;
}

function validategeneratedat(value) {
  const result = validatePayload({ 'generatedAt': value }, { partial: true });
  return result.errors.filter(error => error.field === 'generatedAt');
}

function describegeneratedat() {
  return describeParameter('generatedAt');
}

function defaultgeneratedat() {
  return createDefaultParameters()['generatedAt'];
}

const generatedatParameter = Object.freeze({
  name: 'generatedAt',
  definition: PARAMETER_DEFINITIONS['generatedAt'],
  get: getgeneratedat,
  set: setgeneratedat,
  has: hasgeneratedat,
  clear: cleargeneratedat,
  validate: validategeneratedat,
  describe: describegeneratedat,
  defaultValue: defaultgeneratedat
});
...
const PARAMETERS = Object.freeze({
  ...
  requestedby: requestedbyParameter,
  generatedat: generatedatParameter,
...
function parameterPolicy_3(input = {}) {
  const value = normalizeField('parameters', input['parameters']);
  const definition = PARAMETER_DEFINITIONS['parameters'];
  return {
    field: 'parameters',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'parameters' : 'parameters')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_4(input = {}) {
  const value = normalizeField('format', input['format']);
  const definition = PARAMETER_DEFINITIONS['format'];
  return {
    field: 'format',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'format' : 'format')]: value }, { partial: true }).valid
  };
}
...
function parameterPolicy_6(input = {}) {
  const value = normalizeField('fileUrl', input['fileUrl']);
  const definition = PARAMETER_DEFINITIONS['fileUrl'];
  return {
    field: 'fileUrl',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'fileUrl' : 'fileUrl')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_7(input = {}) {
  const value = normalizeField('requestedBy', input['requestedBy']);
  const definition = PARAMETER_DEFINITIONS['requestedBy'];
  return {
    field: 'requestedBy',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'requestedBy' : 'requestedBy')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_8(input = {}) {
  const value = normalizeField('generatedAt', input['generatedAt']);
  const definition = PARAMETER_DEFINITIONS['generatedAt'];
  return {
    field: 'generatedAt',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'generatedAt' : 'generatedAt')]: value }, { partial: true }).valid
  };
}
