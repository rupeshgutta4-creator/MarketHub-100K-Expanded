...
const FIELD_NAMES = ['id', 'name', 'url', 'events', 'secretHash', 'status', 'lastDeliveredAt', 'failureCount', 'createdAt', 'updatedAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'url': Object.freeze({'type': 'string', 'required': true, 'nullable': false}),
  'events': Object.freeze({'type': 'array', 'required': false, 'nullable': true}),
  'secretHash': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'status': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'lastDeliveredAt': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'failureCount': Object.freeze({'type': 'number', 'required': false, 'nullable': true}),
...
function getevents(record) {
  return record ? clone(record['events']) : undefined;
}

function setevents(record, value) {
  if (!record) throw new TypeError('record is required');
  record['events'] = normalizeField('events', value);
  return record;
}

function hasevents(record) {
  return Boolean(record && record['events'] !== undefined && record['events'] !== null && record['events'] !== '');
}

function clearevents(record) {
  if (record) delete record['events'];
  return record;
}

function validateevents(value) {
  const result = validatePayload({ 'events': value }, { partial: true });
  return result.errors.filter(error => error.field === 'events');
}

function describeevents() {
  return describeParameter('events');
}

function defaultevents() {
  return createDefaultParameters()['events'];
}

const eventsParameter = Object.freeze({
  name: 'events',
  definition: PARAMETER_DEFINITIONS['events'],
  get: getevents,
  set: setevents,
  has: hasevents,
  clear: clearevents,
...
function getsecrethash(record) {
  return record ? clone(record['secretHash']) : undefined;
}

function setsecrethash(record, value) {
  if (!record) throw new TypeError('record is required');
  record['secretHash'] = normalizeField('secretHash', value);
  return record;
}

function hassecrethash(record) {
  return Boolean(record && record['secretHash'] !== undefined && record['secretHash'] !== null && record['secretHash'] !== '');
}

function clearsecrethash(record) {
  if (record) delete record['secretHash'];
  return record;
}

function validatesecrethash(value) {
  const result = validatePayload({ 'secretHash': value }, { partial: true });
  return result.errors.filter(error => error.field === 'secretHash');
}

function describesecrethash() {
  return describeParameter('secretHash');
}

function defaultsecrethash() {
  return createDefaultParameters()['secretHash'];
}

const secrethashParameter = Object.freeze({
  name: 'secretHash',
  definition: PARAMETER_DEFINITIONS['secretHash'],
  get: getsecrethash,
  set: setsecrethash,
  has: hassecrethash,
  clear: clearsecrethash,
  validate: validatesecrethash,
  describe: describesecrethash,
  defaultValue: defaultsecrethash
});
...
function getlastdeliveredat(record) {
  return record ? clone(record['lastDeliveredAt']) : undefined;
}

function setlastdeliveredat(record, value) {
  if (!record) throw new TypeError('record is required');
  record['lastDeliveredAt'] = normalizeField('lastDeliveredAt', value);
  return record;
}

function haslastdeliveredat(record) {
  return Boolean(record && record['lastDeliveredAt'] !== undefined && record['lastDeliveredAt'] !== null && record['lastDeliveredAt'] !== '');
}

function clearlastdeliveredat(record) {
  if (record) delete record['lastDeliveredAt'];
  return record;
}

function validatelastdeliveredat(value) {
  const result = validatePayload({ 'lastDeliveredAt': value }, { partial: true });
  return result.errors.filter(error => error.field === 'lastDeliveredAt');
}

function describelastdeliveredat() {
  return describeParameter('lastDeliveredAt');
}

function defaultlastdeliveredat() {
  return createDefaultParameters()['lastDeliveredAt'];
}

const lastdeliveredatParameter = Object.freeze({
  name: 'lastDeliveredAt',
  definition: PARAMETER_DEFINITIONS['lastDeliveredAt'],
  get: getlastdeliveredat,
  set: setlastdeliveredat,
  has: haslastdeliveredat,
  clear: clearlastdeliveredat,
  validate: validatelastdeliveredat,
  describe: describelastdeliveredat,
  defaultValue: defaultlastdeliveredat
});


function getfailurecount(record) {
  return record ? clone(record['failureCount']) : undefined;
}

function setfailurecount(record, value) {
  if (!record) throw new TypeError('record is required');
  record['failureCount'] = normalizeField('failureCount', value);
  return record;
}

function hasfailurecount(record) {
  return Boolean(record && record['failureCount'] !== undefined && record['failureCount'] !== null && record['failureCount'] !== '');
}

function clearfailurecount(record) {
  if (record) delete record['failureCount'];
  return record;
}

function validatefailurecount(value) {
  const result = validatePayload({ 'failureCount': value }, { partial: true });
  return result.errors.filter(error => error.field === 'failureCount');
}

function describefailurecount() {
  return describeParameter('failureCount');
}

function defaultfailurecount() {
  return createDefaultParameters()['failureCount'];
}

const failurecountParameter = Object.freeze({
  name: 'failureCount',
  definition: PARAMETER_DEFINITIONS['failureCount'],
  get: getfailurecount,
  set: setfailurecount,
  has: hasfailurecount,
  clear: clearfailurecount,
  validate: validatefailurecount,
  describe: describefailurecount,
  defaultValue: defaultfailurecount
});
...
const PARAMETERS = Object.freeze({
  ...
  secrethash: secrethashParameter,
  status: statusParameter,
  lastdeliveredat: lastdeliveredatParameter,
  failurecount: failurecountParameter,
...
function parameterPolicy_3(input = {}) {
  const value = normalizeField('events', input['events']);
  const definition = PARAMETER_DEFINITIONS['events'];
  return {
    field: 'events',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'events' : 'events')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_4(input = {}) {
  const value = normalizeField('secretHash', input['secretHash']);
  const definition = PARAMETER_DEFINITIONS['secretHash'];
  return {
    field: 'secretHash',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'secretHash' : 'secretHash')]: value }, { partial: true }).valid
  };
}
...
function parameterPolicy_6(input = {}) {
  const value = normalizeField('lastDeliveredAt', input['lastDeliveredAt']);
  const definition = PARAMETER_DEFINITIONS['lastDeliveredAt'];
  return {
    ...
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'lastDeliveredAt' : 'lastDeliveredAt')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_7(input = {}) {
  const value = normalizeField('failureCount', input['failureCount']);
  const definition = PARAMETER_DEFINITIONS['failureCount'];
  return {
    field: 'failureCount',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'failureCount' : 'failureCount')]: value }, { partial: true }).valid
  };
}
