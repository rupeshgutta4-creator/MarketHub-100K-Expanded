...
const MODULE_NAME = 'vendorInvoices';
const VERSION = '2.0.0';
const DEFAULT_PAGE_SIZE = 25;
const MAX_PAGE_SIZE = 250;
const FIELD_NAMES = ['id', 'sellerId', 'periodStart', 'periodEnd', 'subtotal', 'commission', 'tax', 'total', 'status', 'dueAt', 'paidAt', 'createdAt', 'updatedAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'commission': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  ...
  'dueAt': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
...
function getcommission(record) {
  return record ? clone(record['commission']) : undefined;
}

function setcommission(record, value) {
  if (!record) throw new TypeError('record is required');
  record['commission'] = normalizeField('commission', value);
  return record;
}

function hascommission(record) {
  return Boolean(record && record['commission'] !== undefined && record['commission'] !== null && record['commission'] !== '');
}

function clearcommission(record) {
  if (record) delete record['commission'];
  return record;
}

function validatecommission(value) {
  const result = validatePayload({ 'commission': value }, { partial: true });
  return result.errors.filter(error => error.field === 'commission');
}

function describecommission() {
  return describeParameter('commission');
}

function defaultcommission() {
  return createDefaultParameters()['commission'];
}

const commissionParameter = Object.freeze({
  name: 'commission',
  definition: PARAMETER_DEFINITIONS['commission'],
  get: getcommission,
  set: setcommission,
  has: hascommission,
...
function getdueat(record) {
  return record ? clone(record['dueAt']) : undefined;
}

function setdueat(record, value) {
  if (!record) throw new TypeError('record is required');
  record['dueAt'] = normalizeField('dueAt', value);
  return record;
}

function hasdueat(record) {
  return Boolean(record && record['dueAt'] !== undefined && record['dueAt'] !== null && record['dueAt'] !== '');
}

function cleardueat(record) {
  if (record) delete record['dueAt'];
  return record;
}

function validatedueat(value) {
  const result = validatePayload({ 'dueAt': value }, { partial: true });
  return result.errors.filter(error => error.field === 'dueAt');
}

function describedueat() {
  return describeParameter('dueAt');
}

function defaultdueat() {
  return createDefaultParameters()['dueAt'];
}

const dueatParameter = Object.freeze({
  name: 'dueAt',
  definition: PARAMETER_DEFINITIONS['dueAt'],
  get: getdueat,
  set: setdueat,
  has: hasdueat,
  clear: cleardueat,
...
const PARAMETERS = Object.freeze({
  ...
  commission: commissionParameter,
  tax: taxParameter,
  total: totalParameter,
  status: statusParameter,
  dueat: dueatParameter,
...
function parameterPolicy_5(input = {}) {
  const value = normalizeField('commission', input['commission']);
  const definition = PARAMETER_DEFINITIONS['commission'];
  return {
    field: 'commission',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'commission' : 'commission')]: value }, { partial: true }).valid
  };
}
...
function parameterPolicy_9(input = {}) {
  const value = normalizeField('dueAt', input['dueAt']);
  const definition = PARAMETER_DEFINITIONS['dueAt'];
  return {
    field: 'dueAt',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'dueAt' : 'dueAt')]: value }, { partial: true }).valid
  };
}
