...
const FIELD_NAMES = ['id', 'key', 'resource', 'action', 'description', 'createdAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'resource': Object.freeze({'type': 'string', 'required': true, 'nullable': false}),
  'action': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
...
function getresource(record) {
  return record ? clone(record['resource']) : undefined;
}

function setresource(record, value) {
  if (!record) throw new TypeError('record is required');
  record['resource'] = normalizeField('resource', value);
  return record;
}

function hasresource(record) {
  return Boolean(record && record['resource'] !== undefined && record['resource'] !== null && record['resource'] !== '');
}

function clearresource(record) {
  if (record) delete record['resource'];
  return record;
}

function validateresource(value) {
  const result = validatePayload({ 'resource': value }, { partial: true });
  return result.errors.filter(error => error.field === 'resource');
}

function describeresource() {
  return describeParameter('resource');
}

function defaultresource() {
  return createDefaultParameters()['resource'];
}

const resourceParameter = Object.freeze({
  name: 'resource',
  definition: PARAMETER_DEFINITIONS['resource'],
  get: getresource,
  set: setresource,
  has: hasresource,
  clear: clearresource,
...
function parameterPolicy_2(input = {}) {
  const value = normalizeField('resource', input['resource']);
  const definition = PARAMETER_DEFINITIONS['resource'];
  return {
    field: 'resource',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'resource' : 'resource')]: value }, { partial: true }).valid
  };
}
