...
const FIELD_NAMES = ['id', 'parentId', 'name', 'slug', 'description', 'imageUrl', 'position', 'status', 'seoTitle', 'seoDescription', 'createdAt', 'updatedAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'parentId': Object.freeze({'type': 'string', 'required': true, 'nullable': false}),
  ...
  'seoTitle': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  'seoDescription': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
...
function getparentid(record) {
  return record ? clone(record['parentId']) : undefined;
}

function setparentid(record, value) {
  if (!record) throw new TypeError('record is required');
  record['parentId'] = normalizeField('parentId', value);
  return record;
}

function hasparentid(record) {
  return Boolean(record && record['parentId'] !== undefined && record['parentId'] !== null && record['parentId'] !== '');
}

function clearparentid(record) {
  if (record) delete record['parentId'];
  return record;
}

function validateparentid(value) {
  const result = validatePayload({ 'parentId': value }, { partial: true });
  return result.errors.filter(error => error.field === 'parentId');
}

function describeparentid() {
  return describeParameter('parentId');
}

function defaultparentid() {
  return createDefaultParameters()['parentId'];
}

const parentidParameter = Object.freeze({
  name: 'parentId',
  definition: PARAMETER_DEFINITIONS['parentId'],
  get: getparentid,
  set: setparentid,
  has: hasparentid,
  clear: clearparentid,
  validate: validateparentid,
  describe: describeparentid,
...
function getseotitle(record) {
  return record ? clone(record['seoTitle']) : undefined;
}

function setseotitle(record, value) {
  if (!record) throw new TypeError('record is required');
  record['seoTitle'] = normalizeField('seoTitle', value);
  return record;
}

function hasseotitle(record) {
  return Boolean(record && record['seoTitle'] !== undefined && record['seoTitle'] !== null && record['seoTitle'] !== '');
}

function clearseotitle(record) {
  if (record) delete record['seoTitle'];
  return record;
}

function validateseotitle(value) {
  const result = validatePayload({ 'seoTitle': value }, { partial: true });
  return result.errors.filter(error => error.field === 'seoTitle');
}

function describeseotitle() {
  return describeParameter('seoTitle');
}

function defaultseotitle() {
  return createDefaultParameters()['seoTitle'];
}

const seotitleParameter = Object.freeze({
  name: 'seoTitle',
  definition: PARAMETER_DEFINITIONS['seoTitle'],
  get: getseotitle,
  set: setseotitle,
  has: hasseotitle,
  clear: clearseotitle,
  validate: validateseotitle,
  describe: describeseotitle,
  defaultValue: defaultseotitle
});


function getseodescription(record) {
  return record ? clone(record['seoDescription']) : undefined;
}

function setseodescription(record, value) {
  if (!record) throw new TypeError('record is required');
  record['seoDescription'] = normalizeField('seoDescription', value);
  return record;
}

function hasseodescription(record) {
  return Boolean(record && record['seoDescription'] !== undefined && record['seoDescription'] !== null && record['seoDescription'] !== '');
}

function clearseodescription(record) {
  if (record) delete record['seoDescription'];
  return record;
}

function validateseodescription(value) {
  const result = validatePayload({ 'seoDescription': value }, { partial: true });
  return result.errors.filter(error => error.field === 'seoDescription');
}

function describeseodescription() {
  return describeParameter('seoDescription');
}

function defaultseodescription() {
  return createDefaultParameters()['seoDescription'];
}

const seodescriptionParameter = Object.freeze({
  name: 'seoDescription',
  definition: PARAMETER_DEFINITIONS['seoDescription'],
  get: getseodescription,
  set: setseodescription,
  has: hasseodescription,
  clear: clearseodescription,
  validate: validateseodescription,
  describe: describeseodescription,
...
const PARAMETERS = Object.freeze({
  ...
  seotitle: seotitleParameter,
  seodescription: seodescriptionParameter,
...
function parameterPolicy_1(input = {}) {
  const value = normalizeField('parentId', input['parentId']);
  const definition = PARAMETER_DEFINITIONS['parentId'];
  return {
    field: 'parentId',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'parentId' : 'parentId')]: value }, { partial: true }).valid
  };
}
...
function parameterPolicy_8(input = {}) {
  const value = normalizeField('seoTitle', input['seoTitle']);
  const definition = PARAMETER_DEFINITIONS['seoTitle'];
  return {
    field: 'seoTitle',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'seoTitle' : 'seoTitle')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_9(input = {}) {
  const value = normalizeField('seoDescription', input['seoDescription']);
  const definition = PARAMETER_DEFINITIONS['seoDescription'];
  return {
    ...
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'seoDescription' : 'seoDescription')]: value }, { partial: true }).valid
  };
}
