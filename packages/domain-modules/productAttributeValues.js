...
const MODULE_NAME = 'productAttributeValues';
const VERSION = '2.0.0';
const DEFAULT_PAGE_SIZE = 25;
const MAX_PAGE_SIZE = 250;
const FIELD_NAMES = ['id', 'productId', 'attributeId', 'value', 'createdAt', 'updatedAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'attributeId': Object.freeze({'type': 'string', 'required': true, 'nullable': false}),
...
function getattributeid(record) {
  return record ? clone(record['attributeId']) : undefined;
}

function setattributeid(record, value) {
  if (!record) throw new TypeError('record is required');
  record['attributeId'] = normalizeField('attributeId', value);
  return record;
}

function hasattributeid(record) {
  return Boolean(record && record['attributeId'] !== undefined && record['attributeId'] !== null && record['attributeId'] !== '');
}

function clearattributeid(record) {
  if (record) delete record['attributeId'];
  return record;
}

function validateattributeid(value) {
  const result = validatePayload({ 'attributeId': value }, { partial: true });
  return result.errors.filter(error => error.field === 'attributeId');
}

function describeattributeid() {
  return describeParameter('attributeId');
}

function defaultattributeid() {
  return createDefaultParameters()['attributeId'];
}

const attributeidParameter = Object.freeze({
  name: 'attributeId',
  definition: PARAMETER_DEFINITIONS['attributeId'],
  get: getattributeid,
  set: setattributeid,
  has: hasattributeid,
  clear: clearattributeid,
  validate: validateattributeid,
  describe: describeattributeid,
...
const PARAMETERS = Object.freeze({
  ...
  attributeid: attributeidParameter,
...
function parameterPolicy_2(input = {}) {
  const value = normalizeField('attributeId', input['attributeId']);
  const definition = PARAMETER_DEFINITIONS['attributeId'];
  return {
    field: 'attributeId',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'attributeId' : 'attributeId')]: value }, { partial: true }).valid
  };
}
