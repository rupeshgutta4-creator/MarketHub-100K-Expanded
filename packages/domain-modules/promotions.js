...
const FIELD_NAMES = ['id', 'name', 'description', 'type', 'value', 'startsAt', 'endsAt', 'status', 'priority', 'conditions', 'createdAt', 'updatedAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'conditions': Object.freeze({'type': 'object', 'required': false, 'nullable': true}),
...
function getconditions(record) {
  return record ? clone(record['conditions']) : undefined;
}

function setconditions(record, value) {
  if (!record) throw new TypeError('record is required');
  record['conditions'] = normalizeField('conditions', value);
  return record;
}

function hasconditions(record) {
  return Boolean(record && record['conditions'] !== undefined && record['conditions'] !== null && record['conditions'] !== '');
}

function clearconditions(record) {
  if (record) delete record['conditions'];
  return record;
}

function validateconditions(value) {
  const result = validatePayload({ 'conditions': value }, { partial: true });
  return result.errors.filter(error => error.field === 'conditions');
}

function describeconditions() {
  return describeParameter('conditions');
}

function defaultconditions() {
  return createDefaultParameters()['conditions'];
}

const conditionsParameter = Object.freeze({
  name: 'conditions',
  definition: PARAMETER_DEFINITIONS['conditions'],
  get: getconditions,
  set: setconditions,
  has: hasconditions,
...
function parameterPolicy_9(input = {}) {
  const value = normalizeField('conditions', input['conditions']);
  const definition = PARAMETER_DEFINITIONS['conditions'];
  return {
    field: 'conditions',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'conditions' : 'conditions')]: value }, { partial: true }).valid
  };
}
