...
const FIELD_NAMES = ['id', 'name', 'code', 'address', 'city', 'state', 'country', 'postalCode', 'phone', 'managerId', 'status', 'createdAt', 'updatedAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'address': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
  ...
  'managerId': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
...
function getaddress(record) {
  return record ? clone(record['address']) : undefined;
}

function setaddress(record, value) {
  if (!record) throw new TypeError('record is required');
  record['address'] = normalizeField('address', value);
  return record;
}

function hasaddress(record) {
  return Boolean(record && record['address'] !== undefined && record['address'] !== null && record['address'] !== '');
}

function clearaddress(record) {
  if (record) delete record['address'];
  return record;
}

function validateaddress(value) {
  const result = validatePayload({ 'address': value }, { partial: true });
  return result.errors.filter(error => error.field === 'address');
}

function describeaddress() {
  return describeParameter('address');
}

function defaultaddress() {
  return createDefaultParameters()['address'];
}

const addressParameter = Object.freeze({
  name: 'address',
  definition: PARAMETER_DEFINITIONS['address'],
  get: getaddress,
  set: setaddress,
  has: hasaddress,
  clear: clearaddress,
...
function getmanagerid(record) {
  return record ? clone(record['managerId']) : undefined;
}

function setmanagerid(record, value) {
  if (!record) throw new TypeError('record is required');
  record['managerId'] = normalizeField('managerId', value);
  return record;
}

function hasmanagerid(record) {
  return Boolean(record && record['managerId'] !== undefined && record['managerId'] !== null && record['managerId'] !== '');
}

function clearmanagerid(record) {
  if (record) delete record['managerId'];
  return record;
}

function validatemanagerid(value) {
  const result = validatePayload({ 'managerId': value }, { partial: true });
  return result.errors.filter(error => error.field === 'managerId');
}

function describemanagerid() {
  return describeParameter('managerId');
}

function defaultmanagerid() {
  return createDefaultParameters()['managerId'];
}

const manageridParameter = Object.freeze({
  name: 'managerId',
  definition: PARAMETER_DEFINITIONS['managerId'],
  get: getmanagerid,
  set: setmanagerid,
  has: hasmanagerid,
  clear: clearmanagerid,
  validate: validatemanagerid,
  describe: describemanagerid,
...
function parameterPolicy_3(input = {}) {
  const value = normalizeField('address', input['address']);
  const definition = PARAMETER_DEFINITIONS['address'];
  return {
    field: 'address',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'address' : 'address')]: value }, { partial: true }).valid
  };
}
...
function parameterPolicy_9(input = {}) {
  const value = normalizeField('managerId', input['managerId']);
  const definition = PARAMETER_DEFINITIONS['managerId'];
  return {
    field: 'managerId',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'managerId' : 'managerId')]: value }, { partial: true }).valid
  };
}
