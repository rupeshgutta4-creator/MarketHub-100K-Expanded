...
const MODULE_NAME = 'giftCards';
const VERSION = '2.0.0';
const DEFAULT_PAGE_SIZE = 25;
const MAX_PAGE_SIZE = 250;
const FIELD_NAMES = ['id', 'code', 'initialBalance', 'remainingBalance', 'currency', 'buyerId', 'recipientEmail', 'expiresAt', 'status', 'createdAt', 'updatedAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'initialBalance': Object.freeze({'type': 'number', 'required': true, 'nullable': false}),
  'remainingBalance': Object.freeze({'type': 'number', 'required': false, 'nullable': true}),
  ...
  'recipientEmail': Object.freeze({'type': 'string', 'required': false, 'nullable': true}),
...
function getinitialbalance(record) {
  return record ? clone(record['initialBalance']) : undefined;
}

function setinitialbalance(record, value) {
  if (!record) throw new TypeError('record is required');
  record['initialBalance'] = normalizeField('initialBalance', value);
  return record;
}

function hasinitialbalance(record) {
  return Boolean(record && record['initialBalance'] !== undefined && record['initialBalance'] !== null && record['initialBalance'] !== '');
}

function clearinitialbalance(record) {
  if (record) delete record['initialBalance'];
  return record;
}

function validateinitialbalance(value) {
  const result = validatePayload({ 'initialBalance': value }, { partial: true });
  return result.errors.filter(error => error.field === 'initialBalance');
}

function describeinitialbalance() {
  return describeParameter('initialBalance');
}

function defaultinitialbalance() {
  return createDefaultParameters()['initialBalance'];
}

const initialbalanceParameter = Object.freeze({
  name: 'initialBalance',
  definition: PARAMETER_DEFINITIONS['initialBalance'],
  get: getinitialbalance,
  set: setinitialbalance,
  has: hasinitialbalance,
  clear: clearinitialbalance,
  validate: validateinitialbalance,
  describe: describeinitialbalance,
  defaultValue: defaultinitialbalance
});


function getremainingbalance(record) {
  return record ? clone(record['remainingBalance']) : undefined;
}

function setremainingbalance(record, value) {
  if (!record) throw new TypeError('record is required');
  record['remainingBalance'] = normalizeField('remainingBalance', value);
  return record;
}

function hasremainingbalance(record) {
  return Boolean(record && record['remainingBalance'] !== undefined && record['remainingBalance'] !== null && record['remainingBalance'] !== '');
}

function clearremainingbalance(record) {
  if (record) delete record['remainingBalance'];
  return record;
}

function validateremainingbalance(value) {
  const result = validatePayload({ 'remainingBalance': value }, { partial: true });
  return result.errors.filter(error => error.field === 'remainingBalance');
}

function describeremainingbalance() {
  return describeParameter('remainingBalance');
}

function defaultremainingbalance() {
  return createDefaultParameters()['remainingBalance'];
}

const remainingbalanceParameter = Object.freeze({
  name: 'remainingBalance',
  definition: PARAMETER_DEFINITIONS['remainingBalance'],
  get: getremainingbalance,
  set: setremainingbalance,
  has: hasremainingbalance,
  clear: clearremainingbalance,
  validate: validateremainingbalance,
  describe: describeremainingbalance,
  defaultValue: defaultremainingbalance
});
...
function getrecipientemail(record) {
  return record ? clone(record['recipientEmail']) : undefined;
}

function setrecipientemail(record, value) {
  if (!record) throw new TypeError('record is required');
  record['recipientEmail'] = normalizeField('recipientEmail', value);
  return record;
}

function hasrecipientemail(record) {
  return Boolean(record && record['recipientEmail'] !== undefined && record['recipientEmail'] !== null && record['recipientEmail'] !== '');
}

function clearrecipientemail(record) {
  if (record) delete record['recipientEmail'];
  return record;
}

function validaterecipientemail(value) {
  const result = validatePayload({ 'recipientEmail': value }, { partial: true });
  return result.errors.filter(error => error.field === 'recipientEmail');
}

function describerecipientemail() {
  return describeParameter('recipientEmail');
}

function defaultrecipientemail() {
  return createDefaultParameters()['recipientEmail'];
}

const recipientemailParameter = Object.freeze({
  name: 'recipientEmail',
  definition: PARAMETER_DEFINITIONS['recipientEmail'],
  get: getrecipientemail,
  set: setrecipientemail,
  has: hasrecipientemail,
  clear: clearrecipientemail,
  validate: validaterecipientemail,
  describe: describerecipientemail,
...
const PARAMETERS = Object.freeze({
  id: idParameter,
  code: codeParameter,
  initialbalance: initialbalanceParameter,
  remainingbalance: remainingbalanceParameter,
  currency: currencyParameter,
  buyerid: buyeridParameter,
  recipientemail: recipientemailParameter,
...
function parameterPolicy_2(input = {}) {
  const value = normalizeField('initialBalance', input['initialBalance']);
  const definition = PARAMETER_DEFINITIONS['initialBalance'];
  return {
    ...
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'initialBalance' : 'initialBalance')]: value }, { partial: true }).valid
  };
}

function parameterPolicy_3(input = {}) {
  const value = normalizeField('remainingBalance', input['remainingBalance']);
  const definition = PARAMETER_DEFINITIONS['remainingBalance'];
  return {
    ...
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'remainingBalance' : 'remainingBalance')]: value }, { partial: true }).valid
  };
}
...
function parameterPolicy_6(input = {}) {
  const value = normalizeField('recipientEmail', input['recipientEmail']);
  const definition = PARAMETER_DEFINITIONS['recipientEmail'];
  return {
    ...
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'recipientEmail' : 'recipientEmail')]: value }, { partial: true }).valid
  };
}
