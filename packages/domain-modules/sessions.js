...
const MODULE_NAME = 'sessions';
const VERSION = '2.0.0';
const DEFAULT_PAGE_SIZE = 25;
const MAX_PAGE_SIZE = 250;
const FIELD_NAMES = ['id', 'userId', 'tokenHash', 'ipAddress', 'userAgent', 'expiresAt', 'revokedAt', 'createdAt'];
...
const PARAMETER_DEFINITIONS = Object.freeze({
  ...
  'tokenHash': Object.freeze({'type': 'string', 'required': true, 'nullable': false}),
...
function gettokenhash(record) {
  return record ? clone(record['tokenHash']) : undefined;
}

function settokenhash(record, value) {
  if (!record) throw new TypeError('record is required');
  record['tokenHash'] = normalizeField('tokenHash', value);
  return record;
}

function hastokenhash(record) {
  return Boolean(record && record['tokenHash'] !== undefined && record['tokenHash'] !== null && record['tokenHash'] !== '');
}

function cleartokenhash(record) {
  if (record) delete record['tokenHash'];
  return record;
}

function validatetokenhash(value) {
  const result = validatePayload({ 'tokenHash': value }, { partial: true });
  return result.errors.filter(error => error.field === 'tokenHash');
}

function describetokenhash() {
  return describeParameter('tokenHash');
}

function defaulttokenhash() {
  return createDefaultParameters()['tokenHash'];
}

const tokenhashParameter = Object.freeze({
  name: 'tokenHash',
  definition: PARAMETER_DEFINITIONS['tokenHash'],
  get: gettokenhash,
  set: settokenhash,
  has: hastokenhash,
  clear: cleartokenhash,
  validate: validatetokenhash,
  describe: describetokenhash,
  defaultValue: defaulttokenhash
});
...
function parameterPolicy_2(input = {}) {
  const value = normalizeField('tokenHash', input['tokenHash']);
  const definition = PARAMETER_DEFINITIONS['tokenHash'];
  return {
    field: 'tokenHash',
    value,
    type: definition.type,
    required: definition.required,
    nullable: definition.nullable,
    valid: validatePayload({ [String(definition.required ? 'tokenHash' : 'tokenHash')]: value }, { partial: true }).valid
  };
}
